...
public class Plc {
    ...
    private static final String INVALID_SEARCH_CRITERIA_MESSAGE = "Invalid criteria: %s";
    private static final String NO_RESULTS_MESSAGE = "No measures found by '%s' matching: %s";
    private static final String RESULTS_HEADER = "Measures found by '%s' matching '%s':";
        ...
        try {
            while (true) {
                ...
                if (searchParam.isEmpty()) {
                    System.out.println(EMPTY_SEARCH_CRITERIA_MESSAGE);
                    continue;
                }
     ...
     */
    private static List<String> performSearch(Category root, SearchType searchType, String searchParam) {
        try {
            ...
            System.out.println(String.format(INVALID_SEARCH_CRITERIA_MESSAGE, e.getMessage()));
    ...
    private static void displayResult(List<String> result, SearchType searchType, String searchParam) {
        if (result.isEmpty()) {
            System.out.println(String.format(NO_RESULTS_MESSAGE, searchType.getValue(), searchParam));
        } else {
            System.out.println(String.format(RESULTS_HEADER, searchType.getValue(), searchParam));
            for (int i = 0; i < result.size(); i++) {
                System.out.println("  [" + (i + 1) + "] " + result.get(i));
            }
            String plural = result.size() > 1 ? "s" : "";
            System.out.println(String.format(RESULT_COUNT, result.size(), plural));
        }
    }
}
